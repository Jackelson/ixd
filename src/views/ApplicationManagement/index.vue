...
<template>
  <div class="applicationStyle">
    <el-row
      style="width: 99%; margin-left: 0.5%; height: calc(100% - 0vh)"
      class="switch-search"
    >
      <el-card class="app-card">
        ...
        <div class="application-btn">
          <!-- <el-row :gutter="10">
            <el-col v-for="n in buttonList" :key="n.name">
    ...
    <el-dialog title="上传安全测试报告" v-model="dialogUploadSave" width="30%">
      ...
      >
        ...
        <template #tip>
          <div class="el-upload__tip">只能上传pdf格式</div>
        </template>
      </el-upload>
    </el-dialog>
    <el-dialog title="上传zip压缩包" v-model="dialogUploadZip" width="30%">
      <el-upload
        ...
        action="#"
        :on-preview="handlePreview"
        :on-change="handleAvatarSuccessZip"
        :on-remove="handleRemove"
        :on-exceed="handleExceed"
        accept=".zip"
        :auto-upload="false"
      ...
      >
        <el-button type="primary">点击上传</el-button>
        <template #tip>
          <div class="el-upload__tip">只能上传zip格式</div>
        </template>
      </el-upload>
    </el-dialog>
    <el-dialog title="删除文件" v-model="dialogDelete" width="30%">
      <el-button
        :disabled="!multipleSelection[0].securityTestingFileId"
        @click="handleDelete('securityTestingFileId')"
        >删除安全测试报告</el-button
      >
      <el-button
        :disabled="!multipleSelection[0].frontFileId"
        @click="handleDelete('frontFileId')"
        >删除压缩包</el-button
      >
    </el-dialog>
  </div>
...
export default {
    ...
    buttonList: function () {
      return [
        ...
        },
        {
          type: "primary",
          name: "撤销",
          event: this.handleRevoke,
          disabled: this.isRevoke,
        },
        {
          type: "primary",
          name: "应用上架",
          event: this.handleGrounding,
          disabled: this.isGrounding,
        },
        {
          type: "primary",
          name: "应用下架",
          event: this.handleOff,
          disabled: this.isOff,
        },
        {
          type: "primary",
        ...
        },
        {
          type: "primary",
          name: "上传安全测试报告",
          event: this.handleUpdateSave,
          disabled: this.isOff,
        },
        {
          type: "primary",
          name: "下载安全测试报告",
          event: this.handleDownSave,
          disabled: this.isOff,
        },
        {
          type: "primary",
          name: "上传压缩包",
          event: this.handleUpdateZip,
          disabled: this.isOff,
        },
        {
          type: "primary",
  ...
  methods: {
    ...
    handleDelete(type) {
      const params = [this.multipleSelection[0][type]];
    ...
    handleDownSave() {
      if (
        !this.multipleSelection[0].securityTestingFileId ||
        this.multipleSelection[0].securityTestingFileId == ""
      ) {
        this.$message({
          type: "warning",
    ...
    handleDownZip() {
      if (
        !this.multipleSelection[0].frontFileId ||
        this.multipleSelection[0].frontFileId == ""
      ) {
        this.$message({
          type: "warning",
    ...
    handleAvatarSuccessZip(file) {
      ...
      let appUrl = this.multipleSelection[0].appIndexUrl.substring(
        0,
        this.multipleSelection[0].appIndexUrl.indexOf("/")
      );
