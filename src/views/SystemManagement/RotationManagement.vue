...
<template>
  <div class="app-container">
      ...
      <el-card class="role-card">
        ...
        <el-row style="width: 100%; display: flex; justify-content: flex-end;">
          <el-pagination :current-page="page" :page-sizes="[10, 20, 50, 100]" :page-size="pageSize"
            layout="total, sizes, prev, pager, next, jumper" :total="imgTotal" @size-change="handleSizeChange"
    ...
    <!--弹框部分-->
    <RotationDialog :title="dialogTitle" :dialog-status="dialogStatus" v-model:show="dialogAdd" :temp1="temp"
      source="task" />
  ...
  <div class="app-container">
      ...
      <el-card class="role-card">
        <el-table ref="multipleTable" :data="noticeList" height="calc(100% - 5vh)"
          ...
          <el-table-column v-for="item in noticeHeader" :key="item.key" :label="item.label" show-overflow-tooltip
            align="center" :min-width="item.minWidth" :width="item.width">
            <template v-slot="scope">
              <span v-if="item.key == 'noticeType'">{{ scope.row[item.key] == 1 ? "通知" : scope.row[item.key] == 2 ? "公告" :
                ""
              }}</span>
    ...
    <!--弹框部分-->
    <NoticeDialog :title="dialogTitle" :dialogStatus="dialogNoticeStatus" v-model:show="dialogNotice" :temp1="temp"
      source="task" />
    ...
    <el-dialog title="查看图片" v-model="dialogShow" :close-on-click-modal="false" style="width:500px">
      <img v-if="dialogShow" :src="imgSrc" style="width: 460px;height:300px;" />
    </el-dialog>
  </div>
</template>
<script>
  ...
  methods: {
    fabuNotice() {
      ElMessageBox.confirm(
        ...
        .then(() => {
          if (this.multipleSelection2.length == 0) return this.$message({ type: "warning", message: "请选择公告" })
          let flag = false;
          ...
          if (flag) return this.$message({ type: 'warning', message: "勾选的公告包含已发布的" })
          const params = []
          this.multipleSelection2.forEach(item => {
            const obj = {
              id: item.id,
    ...
    //编辑公告
    noticeUpdate() {
      if (this.multipleSelection2.length > 1) return this.$message({ type: "error", message: "只能选一个公告" })
    ...
    handleStop(status) {
      ...
      ElMessageBox.confirm(
        ...
        .then(() => {
          if (this.multipleSelection.length == 0) return this.$message({ type: "warning", message: "请选择轮播图" })
          const params = []
    ...
    handleUpdateStatus(status) {
      ...
      ElMessageBox.confirm(
        ...
        .then(() => {
          ...
          if (flag == 1) {
            return this.$message({ type: "warning", message: "勾选的轮播图中包含已展示的" })
          } else if (flag == 2) {
            return this.$message({ type: "warning", message: "勾选的轮播图中包含待展示的" })
          }
          const params = []
          this.multipleSelection.forEach(item => {
            const obj = {
