    ...
    >
      <div v-for="(item, index) in formHeader" :key="index">
        <el-form-item :label="item.label" :prop="item.key">
          <el-upload
            v-if="item.key === 'file' && dialogStatus == 'create'"
            class="upload-demo"
            action="#"
          ...
          <img
            :src="'data:image/png;base64,' + this.temp.base64Image"
            v-else-if="item.key === 'file' && dialogStatus == 'update'"
          />
          ...
          <el-input
            v-else-if="item.key == 'imageDesc'"
            type="textarea"
            v-model="temp[item.key]"
            style="width: 90%"
          />
          <el-input
            v-else-if="item.key == 'sort'"
            type="number"
            v-model="temp[item.key]"
            style="width: 90%"
          />
  ...
  data() {
    return {
      ...
      rules: {
        ...
        file: [{ required: true, message: "请上传图片文件", trigger: "blur" }],
      },
      temp: {},
      // file:'',
      createId: "",
